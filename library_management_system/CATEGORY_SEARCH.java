...
public class CATEGORY_SEARCH extends JFrame {
    ...
    public CATEGORY_SEARCH() {
        initComponents();
        UiTheme.installFrameChrome(this, "Category Search", "Browse books by category prefix");
    }

    private void initComponents() {
        setTitle("Search Books By Category");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        results.setEditable(false);

        JPanel top = new JPanel(new GridLayout(1, 2, 8, 8));
        top.setBorder(BorderFactory.createEmptyBorder(12, 12, 12, 12));
        top.add(new JLabel("CATEGORY PREFIX"));
        top.add(searchText);
    ...
    private void searchBooks() {
        try {
            List<Map<String, String>> books = FirebaseBootstrap.searchBooksByCategoryPrefix(searchText.getText().trim());
